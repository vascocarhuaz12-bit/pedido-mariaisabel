const datosCliente = {

    nombre: "María Isabel",

    fotoPrincipal: "imagenes/principal.PNG",

    mensajePortada:
        "Hoy quiero celebrar a una mujer muy especial. ❤️",

    tituloHistoria:
        "Una mujer que admiro profundamente",

    mensajeHistoria:
        "María Isabel, eres una mujer guerrera, linda, cariñosa, romántica, alegre y divertida. Una mujer que admiro profundamente y que siempre tendrá un lugar especial en mi corazón.",

    fechaCumpleanos: "2026-08-19T00:00:00",

    musica: "musica/cancion.mp3",

    carta: {

        saludo: "Mi María Isabel ❤️",

        parrafo1:
            "Hoy quiero celebrar tu vida y recordarte lo especial que eres para mí. Admiro tu fuerza, tu alegría, tu cariño y esa manera tan especial que tienes de ser.",

        parrafo2:
            "También extraño muchísimo a nuestras hijas, Aleshka y Luna. Ellas están contigo y siempre estarán presentes en mi corazón.",

        parrafo3:
            "En este día tan especial quiero agradecerte por todo y recordarte algo que nunca cambiará: pase lo que pase, siempre estarás en mi corazón.",

        firma:
            "Con todo mi amor,\nCarlos Valencia ❤️"
    },

    momentos: [

        {
            fecha: "UNA MUJER ESPECIAL",
            titulo: "María Isabel ❤️",
            texto: "Una mujer guerrera, linda, cariñosa, romántica, alegre y divertida.",
            foto: "imagenes/foto3.PNG"
        },

        {
            fecha: "RECUERDOS",
            titulo: "Momentos que quedan",
            texto: "Cada recuerdo compartido tiene un lugar especial en mi corazón.",
            foto: "imagenes/foto2.PNG"
        },

        {
            fecha: "NUESTRA FAMILIA",
            titulo: "Aleshka y Luna ❤️",
            texto: "Las extraño muchísimo. Ellas también forman parte de todo lo que llevo en mi corazón.",
            foto: "imagenes/foto1.PNG"
        },

        {
            fecha: "19 DE AGOSTO",
            titulo: "Feliz cumpleaños ❤️",
            texto: "Hoy celebramos tu vida y a la maravillosa mujer que eres. Que nunca pierdas tu alegría y tu fuerza.",
            foto: "imagenes/foto4.PNG"
        }

    ]

};